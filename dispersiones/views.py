    ...
    if is_contabilidad:
        ...
        if is_ejecutivo:
            ...
            if puede_ver_todos:
                ...
                ).filter(dispersion__fecha__month=mes, dispersion__fecha__year=anio).filter(servicio__in=["PROCOM", "PRAIDS"]).distinct()
            estatus_pago = ""
...
def dispersiones_kanban(request):
    ...
    clientes_qs = Cliente.objects.filter(dispersion__fecha__year=anio).filter(servicio__in=["PROCOM", "PRAIDS"])
    if mes is not None:
        clientes_qs = clientes_qs.filter(dispersion__fecha__month=mes)
